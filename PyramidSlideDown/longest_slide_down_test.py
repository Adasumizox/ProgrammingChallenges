from longest_slide_down import longest_slide_down
import unittest

class TestLongestSlideDown(unittest.TestCase):
    
    def test(self):
        self.assertEqual(longest_slide_down([[3], [7, 4], [2, 4, 6], [8, 5, 9, 3]]), 23, 'should work for small pyramids')
        ...
        ]), 7273, 'should work for huge pyramids')
        

    def test_rand(self):
        from random import randint
        for _ in range(5):
            pyramid = [[randint(0, 10**5) for x in range(y + 1)] for y in range(100)]
            # Call longest_slide_down first so there's no answer
            # available in a variable (just in case).
            # Pass a copy of pyramid in case longest_slide_down
            # modifies it.
            result = longest_slide_down(list(map(list, pyramid)))
            sums = pyramid[-1]
            for row in pyramid[-2::-1]:
                sums = [a + max(b, c) for (a, b, c) in zip(row, sums, sums[1:])]
            answer = sums[0]
