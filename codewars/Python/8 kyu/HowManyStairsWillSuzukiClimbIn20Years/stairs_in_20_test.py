from stairs_in_20 import stairs_in_20
import unittest
class TestStairsIn20(unittest.TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(stairs_in_20(stairs), 54636040)

    def test_rand(self):
        import random
        random_test = lambda :[random.randint(5000, 10000) for x in range(52)]

        def stairs_in_20_test(stairs):	
            one_yr_total = 0
            for i in stairs:
                for j in i:
                    one_yr_total += j
            return one_yr_total * 20

        for _ in range(100):
            sunday = random_test()
            monday = random_test()
            tuesday = random_test()
            wednesday = random_test()
            thursday = random_test()
            friday = random_test()
            saturday = random_test()
            stairs = [sunday,monday,tuesday,wednesday,thursday,friday,saturday]
            self.assertEqual(stairs_in_20(stairs), stairs_in_20_test(stairs),"It should work with random inputs too" )
